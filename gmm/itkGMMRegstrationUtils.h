#ifndef itkGMMRegstrationUtils_h
#define itkGMMRegstrationUtils_h
...
template<typename T>
void GaussianAffinityMatrix(const T* A, const T* B, int m, int n, int dim, T scale, T* dist) 
{
  scale = -2.0*SQR(scale);
...
T GaussTransform(const T* A, const T* B, int m, int n, int dim, T scale, T* grad) 
{
  ...
  for (int i = 0; i < m; ++i) {
    for (int j = 0; j < n; ++j) {
      ...
      for (int d = 0; d < dim; ++d) {
        dist_ij += pow(A[i * dim + d] - B[j * dim + d], 2);
      }
...
template<typename T>
T GaussTransform(const T* A, const T* B, const int m, const int n, const int dim, const T Ascale, const T Bscale, T* grad)
{
  T value = 0;
  ...
  for (size_t i = 0; i < m; ++i) {
    for (size_t j = 0; j < n; ++j) {
      ...
      for (size_t d = 0; d < dim; ++d) {
        dist += pow(A[i * dim + d] / Ascale - B[j * dim + d] / Bscale, 2);
      }
      ...
      for (size_t d = 0; d < dim; ++d) {
        grad[i * dim + d] -= 2.0 * weight * (A[i * dim + d] / Ascale - B[j * dim + d] / Bscale);
      }
    }
  }
...
T GaussTransform(const vnl_matrix<T>& A, const vnl_matrix<T>& B, T scale) 
{
  ...
  return GaussTransform(A.data_block(), B.data_block(), A.rows(), B.rows(), A.cols(), scale);
}

template<typename T>
T GaussTransform(const vnl_matrix<T>& A, const vnl_matrix<T>& B, T scale, vnl_matrix<T>& gradient) 
{

  return GaussTransform(A.data_block(), B.data_block(), A.rows(), B.rows(), A.cols(), scale, gradient.data_block());
}

// todo: add one more version when the model is same as ctrl_pts
// reference:  Landmark-based Image Analysis, Karl Rohr, p195
template<typename T>
void ComputeTPSKernel(const vnl_matrix<T>& model, const vnl_matrix<T>& ctrl_pts, vnl_matrix<T>& U, vnl_matrix<T>& K) 
{
  int m = model.rows();
  int n = ctrl_pts.rows();
  int d = ctrl_pts.cols();
...
/*
   ...
   */
template<typename T>
void ComputeGaussianKernel(const vnl_matrix<T>& model, const vnl_matrix<T>& ctrl_pts, vnl_matrix<T>& G, vnl_matrix<T>& K, T lambda) 
{
  int m,n,d;
  m = model.rows();
  n = ctrl_pts.rows();
...
template<typename T>
void ComputeSquaredDistanceMatrix(const vnl_matrix<T>& A, const vnl_matrix<T>& B, vnl_matrix<T>& D) 
{
  int m = A.rows();
  int n = B.rows();
